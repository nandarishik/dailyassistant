...
Z_SCORE_PEAK_THRESHOLD = 2.0  # optional high-side alert (peaks)
...
def detect_anomalies(
    ...
    for outlet, daily_series in outlet_data.items():
        ...
        for s in stats:
            ...
            elif s["z_score"] > Z_SCORE_PEAK_THRESHOLD:
                pct_dev = ((s["revenue"] - s["rolling_mean"]) / s["rolling_mean"]) * 100
                anomalies.append(AnomalyRecord(
                    date          = s["date"],
                    outlet_name   = outlet,
    ...
    # Dips first (most negative Z), then peaks (highest Z)
    anomalies.sort(
        key=lambda a: (a.severity == "PEAK", a.z_score if a.severity != "PEAK" else -a.z_score)
    )
    return anomalies
