...
def _get_smtp_config() -> dict:
    ...
    try:
        ...
        if hasattr(st, "secrets"):
            ...
            return {
                "host"      : secrets.get("SMTP_HOST",       os.getenv("SMTP_HOST", os.getenv("SMTP_SERVER", "smtp.gmail.com"))),
                "port"      : int(secrets.get("SMTP_PORT",   os.getenv("SMTP_PORT", "587"))),
                "user"      : secrets.get("SMTP_USER",       os.getenv("SMTP_USER", os.getenv("SMTP_USERNAME", ""))),
                "password"  : secrets.get("SMTP_PASS",       os.getenv("SMTP_PASS", os.getenv("SMTP_PASSWORD", ""))),
                "recipient" : secrets.get("ALERT_RECIPIENT",  os.getenv("ALERT_RECIPIENT", os.getenv("MAIL_TO", ""))),
                "mail_from" : secrets.get("MAIL_FROM",       os.getenv("MAIL_FROM", "")),
            }
    except Exception:
        pass   # Not running in Streamlit, or no secrets file — fall through to env vars

    return {
        "host"      : os.getenv("SMTP_HOST", os.getenv("SMTP_SERVER", "smtp.gmail.com")),
        "port"      : int(os.getenv("SMTP_PORT", "587")),
        "user"      : os.getenv("SMTP_USER", os.getenv("SMTP_USERNAME", "")),
        "password"  : os.getenv("SMTP_PASS", os.getenv("SMTP_PASSWORD", "")),
        "recipient" : os.getenv("ALERT_RECIPIENT", os.getenv("MAIL_TO", "")),
...
def _build_anomaly_rows_html(anomalies: list[dict]) -> str:
    ...
    for a in anomalies:
        ...
        date_s = _esc(a.get("date", "—"))
        outlet_s = _esc(str(a.get("outlet_name", "—"))[:24])

        rows.append(f"""
        <tr style="border-bottom:1px solid #1e293b">
            <td style="padding:12px 8px;color:#e2e8f0;font-weight:600;font-size:14px">{date_s}</td>
            <td style="padding:12px 8px;color:#cbd5e1;font-size:13px">{outlet_s}</td>
...
def _build_diagnosis_html(diagnosis_paragraphs: list[dict]) -> str:
    ...
    for d in diagnosis_paragraphs:
        ...
        on = _esc(d.get("outlet_name", "Unknown"))
        dt = _esc(d.get("date", "—"))
        diag_raw = d.get("diagnosis", "Diagnosis pending...")
        diag_html = _esc(diag_raw).replace("\n", "<br/>")
        blocks.append(f"""
...
def _build_recommendation_html(recommendations: str) -> str:
    ...
    for line in lines:
        ...
        if line.startswith("#") or line.startswith("Top") or line.startswith("Live"):
            html_lines.append(
                f'<div style="font-size:14px;font-weight:700;color:#f59e0b;margin:8px 0 4px">{safe}</div>')
        elif line.startswith("  #"):
            html_lines.append(
                f'<div style="font-size:13px;color:#e2e8f0;padding:6px 0 2px;'
                f'border-bottom:1px solid rgba(255,255,255,0.05)">{safe}</div>')
        else:
            html_lines.append(
                f'<div style="font-size:12px;color:#94a3b8;padding:2px 0 2px 16px">{safe}</div>')
...
def send_email(html_body: str, subject: str = "") -> tuple[bool, str]:
    ...
    if not config["user"] or not config["password"]:
        return False, (
            "SMTP credentials not configured. Set SMTP_USER (or SMTP_USERNAME) and "
            "SMTP_PASS (or SMTP_PASSWORD) in your .env file or Streamlit secrets."
        )

    if not config["recipient"]:
        return False, (
            "No recipient configured. Set ALERT_RECIPIENT or MAIL_TO in your .env file "
    ...
    msg["Subject"] = subject
    msg["From"]    = (config.get("mail_from") or "").strip() or config["user"]
