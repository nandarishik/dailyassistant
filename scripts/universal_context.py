...
Environment variables (load order: repository `.env`, then parent `.env`):
...
class LLMManager:
    ...
    def generate(
        ...
        *,
        ...
        Optional generation controls (used by Copilot planner vs synthesis):
        - `temperature` — passed to both engines when supported.
        - `max_output_tokens` — Gemini `max_output_tokens` (preferred for Gemini).
        - `max_tokens` — OpenRouter cap; if only one of max_tokens / max_output_tokens
          is set, the other engine reuses that numeric cap as a reasonable default.
        """
        ev_log: list[str] = []
        temp = 0.25 if temperature is None else float(temperature)
        gem_cap = (
            int(max_output_tokens)
            if max_output_tokens is not None
            else int(max_tokens or 2048)
        )
        or_cap = int(max_tokens) if max_tokens is not None else int(max_output_tokens or 512)

        # ── Step 1: Try Primary Engine (Gemini) ──────────────────────────
        gemini_result = self._try_gemini(
            prompt, ev_log, temperature=temp, max_output_tokens=gem_cap
        )
        if gemini_result:
        ...
        or_result = self._try_openrouter(prompt, ev_log, temperature=temp, max_tokens=or_cap)
        if or_result:
...
def run_context_engine() -> None:
    ...
    log(f"  .env loaded    : {', '.join(str(p) for p in _ENV_LOAD_PATHS) or '(none — process env only)'}")
    log()
    log("  API status:")
    ...
    _g = GEMINI_KEY[:8] + "…" if len(GEMINI_KEY) >= 8 else (GEMINI_KEY or "—")
    _o = OPENROUTER_KEY[:12] + "…" if len(OPENROUTER_KEY) >= 12 else (OPENROUTER_KEY or "—")
    log(f"    Gemini (Primary)   : {'✓ ' + _g    if GEMINI_KEY     else '✗ missing'}")
    log(f"    OpenRouter (Backup): {'✓ ' + _o if OPENROUTER_KEY else '✗ missing'}")
    log()
